...
import { useImportRoutineMutation, type ExamBatch } from "@/redux/api/examsApi";
...
/** Manual single routine row: creates one scheduled (course-less) draft exam
 *  under the batch (0 questions), then hands off to the question editor. */
export default function AddRoutineExamModal({
  batchId,
  onClose,
  onAdded,
}: {
  ...
  const [importRoutine, { isLoading }] = useImportRoutineMutation();

  const save = async () => {
    if (!title.trim()) return;
    setError(null);
    try {
      const { batch } = await importRoutine({
        batchId,
        publish: false,
        exams: [
          {
            ...
            planned_questions: Number(numQuestions) || 0,
      ...
      <div className="flex max-h-[92vh] w-full max-w-[560px] flex-col overflow-hidden rounded-3xl border border-slate-800 bg-slate-900" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-800 px-6 py-4">
          <h3 className="flex items-center gap-2 text-base font-bold text-blue-50">
            <CalendarClock size={18} className="text-cyan-400" /> রুটিনে নতুন পরীক্ষা
          </h3>
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-6">
          ...
          <div>
            ...
            <input className={fieldClass} placeholder="যেমন: মডেল টেস্ট - ১" value={title} onChange={(e) => setTitle(e.target.value)} />
          </div>
          <div>
          ...
          <p className="text-xs text-slate-500">
            সংরক্ষণের পর রুটিনের সারি থেকে &quot;প্রশ্ন যোগ করুন&quot; চেপে প্রশ্ন (ম্যানুয়ালি বা CSV) যোগ করে প্রকাশ করুন।
          </p>
        </div>
        ...
        <div className="flex justify-end gap-2.5 border-t border-slate-800 px-6 py-4">
          ...
          >
            {isLoading ? "সংরক্ষণ হচ্ছে…" : "যোগ করুন"}
          </button>
        </div>
      </div>
    </div>
