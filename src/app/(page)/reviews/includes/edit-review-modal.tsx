...
import { useUpdateReviewMutation, type Review } from "@/redux/api/contentApi";
...
export default function EditReviewModal({
  ...
  const [courseId, setCourseId] = useState<string>(String(review.course));
  const [studentId, setStudentId] = useState<string>(String(review.student));
  const [rating, setRating] = useState<number>(review.rating);
  const [comment, setComment] = useState(review.comment);
  const [isFeatured, setIsFeatured] = useState(review.is_featured);
      ...
      <div className="relative w-full max-w-xl rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-lg font-bold text-blue-50">রিভিউ সম্পাদনা করুন</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mt-5 flex flex-col gap-4">
          ...
          <div className="mt-2 flex items-center justify-end gap-3 border-t border-slate-800 pt-4">
            ...
            >
              {isLoading ? "সংরক্ষণ করা হচ্ছে…" : "আপডেট করুন"}
            </button>
          </div>
        </form>
      </div>
