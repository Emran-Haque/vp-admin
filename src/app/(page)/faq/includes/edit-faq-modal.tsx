...
import { useUpdateFaqMutation, type Faq } from "@/redux/api/contentApi";
...
export default function EditFaqModal({
  ...
  const [relatedCourse, setRelatedCourse] = useState<string>(
    faq.related_course ? String(faq.related_course) : ""
  );
  const [relatedBook, setRelatedBook] = useState<string>(
    faq.related_book ? String(faq.related_book) : ""
  );
  const [ordering, setOrdering] = useState(faq.ordering ?? 0);
