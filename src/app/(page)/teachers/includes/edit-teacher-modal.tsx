...
export default function EditTeacherModal({ teacher, onClose }: { teacher: Teacher; onClose: () => void }) {
  ...
  const [ordering, setOrdering] = useState(String(teacher.ordering));
  const [isActive, setIsActive] = useState(teacher.is_active);
  const [socialLinks, setSocialLinks] = useState<Record<string, string>>(teacher.social_links || {});
        ...
        <div className="flex flex-1 flex-col gap-3.5 overflow-y-auto p-7">
          ...
          <div className="flex items-center gap-4">
            {previewUrl ? (
              ...
              <img src={previewUrl} alt="preview" className="size-16 shrink-0 rounded-2xl object-cover" />
            ) : (
